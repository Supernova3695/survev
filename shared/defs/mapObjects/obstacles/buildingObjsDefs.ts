...
import { MaterialDefs } from "../mapObjectHelpers.ts";
...
function createDoor(
    overrides: DeepPartial<ObstacleDef>,
    params: {
        hinge: Vec2;
        extents: Vec2;
    },
    ...
    const baseDef: ObstacleDef = {
        ...
        collision: collider.createAabbExtents(params.hinge, params.extents),
        height: 10,
        collidable: true,
        destructible: true,
        health: 150,
    ...
    const material = overrides.material as keyof typeof MaterialDefs;
    if (!MaterialDefs[material]) {
        throw new Error(`Invalid material ${overrides.material}`);
    }
    return util.mergeDeep(baseDef, MaterialDefs[material], overrides);
}

function createLabDoor(overrides: DeepPartial<ObstacleDef>): ObstacleDef {
    const baseDef = createDoor({
        material: "concrete",
        door: {
            interactionRad: 2,
            openOneWay: 0,
...
function createWall(params: DeepPartial<ObstacleDef> & { extents: Vec2 }): ObstacleDef {
    const baseDef: ObstacleDef = {
        type: "obstacle",
        scale: { createMin: 1, createMax: 1, destroy: 1 },
        collision: collider.createAabbExtents(v2.create(0, 0), v2.copy(params.extents)),
        height: 10,
        isWall: true,
        collidable: true,
        destructible: true,
...
function createStairs(overrides: DeepPartial<ObstacleDef>): ObstacleDef {
    const baseDef: ObstacleDef = {
        type: "obstacle",
        scale: { createMin: 1, createMax: 1, destroy: 1 },
...
function createWindow(overrides: DeepPartial<ObstacleDef>): ObstacleDef {
    const baseDef: ObstacleDef = {
        type: "obstacle",
        scale: { createMin: 1, createMax: 1, destroy: 1 },
        collision: collider.createAabbExtents(v2.create(0, 0), v2.create(0.4, 2)),
...
export const BuildingObjectDefs: Record<string, ObstacleDef> = {
